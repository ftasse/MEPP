...
#include "tools/openmeshToPolyhedron.h"
...
void initLUTIndex(double LUT_CourbureClust[])
{
    int i=0;
...
{
    initLUTIndex(LUT_CourbureClust);
...
void Skeleton_Extraction_Component::vertexThicknessToColorMap()
{
         ...
         pVertex++)
    {
        ...
        double R=(pVertex->thickness()-minThickness)/(maxThickness-minThickness)*255;
        indiceLut=floor(std::min(R, 255.0));
        pVertex->color(LUT_CourbureClust[3*indiceLut],
