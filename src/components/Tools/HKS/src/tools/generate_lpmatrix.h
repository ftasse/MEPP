                                               ...
                                               double rho = 3.0 )
{
    ...
    for (v_it= mesh.vertices_begin(); v_it != mesh.vertices_end(); ++v_it)
    {
        ...
        for(; voh_it; ++voh_it)
        {
            ...
            const typename MeshT::Point p1 = mesh.point( mesh.to_vertex_handle( voh_it.handle()));
            const typename MeshT::Point p2 = mesh.point( mesh.from_vertex_handle( mesh.prev_halfedge_handle( voh_it.handle())));
            const typename MeshT::Point p3 = mesh.point( mesh.to_vertex_handle( mesh.next_halfedge_handle( mesh.opposite_halfedge_handle(voh_it.handle()))));
            ...
            wTriplets.push_back(Triplet(v_it.handle().idx(),
                                        mesh.to_vertex_handle(voh_it.handle()).idx(), cotw));
            _weight += cotw;

        }

        wTriplets.push_back(Triplet(v_it.handle().idx(), v_it.handle().idx(), -_weight));
        dTriplets.push_back(Triplet(v_it.handle().idx(), v_it.handle().idx(), 2*_area));
    }
...
void compute_dual_laplacian_matrix_Tutte(MeshT &mesh, SparseMatrix &_Wd)
{
    ...
    for (f_it= mesh.faces_begin(); f_it != mesh.faces_end(); ++f_it)
    {
        ...
        if ( ! fh_it.handle().is_valid() ) continue;

        for(; fh_it; ++fh_it)
        {
            wTriplets.push_back(Triplet(f_it.handle().idx(),
                                        mesh.face_handle(mesh.opposite_halfedge_handle(fh_it.handle())).idx(),
                                        1.0));
            _weight += 1.0;
        }
        wTriplets.push_back(Triplet(f_it.handle().idx(), f_it.handle().idx(), -_weight));
    }
